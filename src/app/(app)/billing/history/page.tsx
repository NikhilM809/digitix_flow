import { markBatchPaid, markInvoicePaid } from "@/actions/billing";
import { AttachInvoicePdf, HistoricalInvoiceUpload } from "@/components/historical-invoice-form";
import { BillingBadge } from "@/components/status";
...
export default async function BillingHistoryPage({
  ...
  const uploadProjects = await prisma.project.findMany({
            ...
            <tbody>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex flex-col items-start gap-2">
                        <div className="flex gap-2">
                          <a href={`/api/invoices/batch/${batch.id}/pdf`} className="text-sm text-teal">
                            PDF
                          </a>
                          {batch.status !== "PAID" ? (
              ...
              {loose.map((invoice) => (
                <tr key={invoice.id} className="border-t border-line">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex gap-2">
                      <a href={`/api/invoices/${invoice.id}/pdf`} className="text-sm text-teal">
                        PDF
                      </a>
                      {invoice.status !== "PAID" ? (
